...
<style>
  ...
  .warn-badge { background: rgba(240,136,62,0.15); color: #f0883e; font-size: 11px;
...
<p class="meta">קונה אחד עם 3+ נמענים שונים — סביר שזה מפיץ. <strong>זו לא כפילות</strong> אלא מידע על מי הזמין עבור מי. נמענים עם ⚠ סומנו ככפילות חשודה (הופיעו גם בקטע 2 או 4).</p>
...
def render_distributor(idx, group, dup_names):
    """dup_names: set of Hebrew names that appear in any fuzzy duplicate cluster.
    Recipients in this set get a ⚠ marker (their name might match another customer)."""
    cid = f"dist_{idx}"
    recipients_html = ""
  ...
  <div class="meta">מפיץ שמזמין עבור לקוחות אחרים — לא כפילות, פשוט סוג של buyer-on-behalf.</div>
</div>'''
...
def main():
    ...
    # Build set of Hebrew names that appear in any fuzzy duplicate cluster
    dup_names = set()
    ...
    dist_html = "".join(
        render_distributor(i, g, dup_names) for i, g in enumerate(data["distributor_groups"])
    ) or "<p>אין מפיצים</p>"
    hemulti_html = "".join(
